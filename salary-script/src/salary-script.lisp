...
(defun handle-salary-script (lst &optional (traceMe t))
	(if (not (atom lst))
		(if (is-if lst)
			(let ((attr (nth 0 lst))
				(condition (nth 1 lst))
			...
			(if (is-let lst)
				(let (
					...
					(let (
							(x (handle-salary-script value nil ) )
							(y (handle-salary-script expr nil ) )
						)
				...
				(if (is-binary lst)
					(let (
							(left (handle-salary-script (nth 0 lst) nil ))
							(op (nth 1 lst) )
							(right (handle-salary-script (nth 2 lst) nil) )
						)
						(if traceMe
						`(make-result-with-trace :trace (quote(,lst)) :result (,op ,left ,right))
						`(,op ,left ,right)
						)
					)
					lst
